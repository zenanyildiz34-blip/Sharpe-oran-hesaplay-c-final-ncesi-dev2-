/*sharpe oranını hesaplayan bir kod yazacağız bunun için kullanıcıdan 3 değer alacağız sonrasında sharpe
 oranı formülüne göre sharpe oranını hesaplayacağız-yanlış karakterler girilmesi durumunda hata komutu ile karşılaşacagız-
...
int main(void) { //c dilinde main fonksiyonu kodu başlatır
    ...
    printf("Risksiz faiz orani (%% cinsinden): ");  /*risksiz faiz oranını ister-bir nevi garantili kazanç kaynaklaırdır-
                                                     yine aynı şekilde sayı şeklinde karakter girişi ister aksi halde hata verir*/
    ...
    printf("Portfoyun standart sapmasi (%% cinsinden): ");  /*portfoyun standrt sapması ise yatırımın ne kadar dalgalandığını 
                                                          ifade eder yüksekse yatırım çok oynak düşükse oynak olmadığını ıfade eder*/
    ...
    if (standart_sapma <= 0) {                             /*formüle göre getiriden risksiz olanı çıkarıp sonucu standart sapmaya 
                                                            bölüyoruz matematiksel açıdan hata çıkmaması adına standart sapma 0 olamaz ayrıca gerçek hayatta
                                                           her yatırım bir miktarda olsa risk içerir yanı 0dan küçük olamaz*/
        printf("Standart sapma 0 veya negatif olamaz.\n"); //yanlı karakter girişlerinde ekranda uyarı verir
        return 1;
    }

    // Sharpe oranı hesaplama
    sharpe_orani = (yatirim_getirisi - risksiz_faiz) / standart_sapma;  //sharpe oranı formülümüz

    printf("\nSharpe Orani = %.2f\n", sharpe_orani);  /*formülden çıkan sonçlarımız ondalıklı olabilir bu yüzden virgülden
                                                        sonraki iki sayıyı görmek için bu satırı yazdık*/
